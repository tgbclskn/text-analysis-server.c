...
int main();
void calc_ptrs_offset_by_size(void **__restrict__ ptr_off_size, void *__restrict__ words_ptr, const int *__restrict__ lettercount);
	...
	if(filesize <= 0)
{
	...
	unsigned long charsread = read(fd,filebuf_ptr,filesize);
	if(charsread == -1)
{
	...
	int lettercount[46]; // Pneumonoultramicroscopicsilicovolcanoconiosis
...
{
	if(sign[s] == -1 && i == inputwords_ptr->len)
		continue;
	...
	if((filesize&shift[i]) == shift[i])
{
	...
	printf("(%s) shift by:%u\n",inputwords_ptr->st,shcou);
	//start binary search
	for(unsigned char i = 0; i < shcou; i++)
{
	//
...
void calc_ptrs_offset_by_size(void **ptr_off_size, void *words_ptr, const int *lettercount) // order in mem by wlen
{
	ptr_off_size[0] = 0;
	for(unsigned char i = 1; i < 46; i++)
{
...
inputstr_s* get_input_words_struct(char* input)
{
	...
	for(unsigned long i = 0, cur = 0; i < inputlen + 1; i++ )
{
	...
	if((i-cur) == 0)
{
	...
	for(unsigned long j = 0; j < inputwords_p->len; j++)
{
	if(!cursor)
{
...
//	getchar();
	...
	unsigned char closest_dist_arr[LEV_SIZE];
...
//FOR
	...
	unsigned char l = levenshtein_n((char*)mem_ptr, w2len, st, wlen);
...
//	printf("(%u)add at %u\n",threadid,i);
	unsigned char dist_tmp1 = l;
	void* ptr_tmp1 = mem_ptr;
	unsigned char dist_tmp2;
	void* ptr_tmp2;
	for(unsigned long j = i; j < LEV_SIZE; j++)
{
...
//	printf("comp:%s\n",(char*)mem_ptr);
//	printf("st:%s\n",st);
//	printf("%s -> %s lev dist:%lu\n",(char*)mem_ptr,st,levenshtein_n((char*)mem_ptr, wlen, st, wlen));
//	getchar();
} //end for mem_ptr
...
//	printf("adding to lev_found[]\n");
	...
	for(unsigned long i = 0; i < LEV_SIZE; i++)
{
	if(closest_ptr_arr[i] == 0)
		break;

	for(unsigned long j = 0; j < LEV_SIZE; j++)
...

{
	...
	void* ptr_tmp1 = closest_ptr_arr[i];
	unsigned char dist_tmp1 = closest_dist_arr[i];
	void* ptr_tmp2;
	unsigned char dist_tmp2;

	for(unsigned long k = j; k < LEV_SIZE; k++)
{
...
/*void pr()
{
	for(unsigned int i = 0; i < LEV_SIZE; i++)
{
	printf("[%u]->str:%s  dist:%u\n",i,(char*)lev_found[i].ptr,lev_found[i].dist);
